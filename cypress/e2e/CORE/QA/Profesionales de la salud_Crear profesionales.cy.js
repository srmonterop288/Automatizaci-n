...
describe("Profesioal de la Salud", () => {
  require("cypress-plugin-tab");
  require("cypress-xpath");

  it("Creación de profesionales de la salud", () => {
    ...
    cy.leerExcel(rutaArchivoExcel).then((datosExcel) => {
      ...
      for (let i = 1; i < datosExcel.length; i++) {
        ...
        cy.wait(1000);
        cy.get("#spn_modulo_medical_records_list_user")
          .should("be.visible")
          .click();
        cy.wait(1000);
        cy.get("#btn_menu_desplegable").should("be.visible").click();
        ...
        if (!DNI || DNI.trim() === "" || DNI.trim() === "undefined") {
          // DNI inválido
          cy.window().then((win) => {
            win.alert("DNI no válido. Finalizando prueba.");
          });
          cy.log("DNI inválido. Se detiene el test.");
          return;
        } else {
          // DNI válido
          cy.window().then((win) => {
            win.alert("DNI válido. Prueba exitosa.");
          });
          cy.log("Prueba finalizada correctamente con DNI válido.");
        }
